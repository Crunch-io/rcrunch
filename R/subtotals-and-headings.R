...
#' @param before character or numeric if `position` is "relative" and also the
...
#' for subtotals only
#' @param na.rm For Multiple Response subtotals, whether to remove missings before
#' calculating the subtotal (so that if `na.rm=TRUE` and there is one missing and
#' one selected, the response would selected instead of missing). This defaults to
#' `TRUE` to match how the crunch web application behaves.
#' @param variable For Multiple Response subtotals, the parent MR variable that
#' contains the subvariables that are being subtotaled (defaults to the variable
#' that is having the Subtotal added to it)
#' @param alias For Multiple Response subtotals, an optional string to use as the
#' new insertion's alias (defaults to letting the server choose the alias)
#' @param id For Multiple Response subtotals, an optional number or string to use
...
setMethod("makeInsertion", "Subtotal", function(x, var_items, alias) {
    ...
    # These can be NULL in R, but don't send NULL to server, so add afterwards
    out$id <- x$id
    out$alias <- x$alias
    ...
    if (!(is.na(func(insert)))) {
        ...
        if (func(insert) == "subtotal") {
            ...
            insert <- Subtotal(
                ...
                id = id(insert), alias = insert$alias
            )
