...
Decks are a way for analysts to save a set of analyses so that they can refer back to it later,
...
exactly right, and so the creator must spell out the fine details of the slide so that their
analysis is clear. The attributes of a slide's analysis consist of:
...
This vignette will start from the pets example dataset.
...
#### Problem
You want to get the frequencies of a single cateorical or multiple response variable.
...
#### Solution
A categorical array contributes two dimensions to the analysis, a "subvariables" dimension
and a "" dimension. If your query just specifies the variable, by default the 
...
The right hand side cannot be empty, but to get the mean of the whole dataset a value of 1
can be used.
```{r}
slide <- newSlide(
    deck, 
...
A query result can have up to three dimensions. The first is the "rows_dimension", second
...
slide <- newSlide(
    deck, 
    ~q1, 
    title = glue("Favorite pet using default palette"),
    display_settings = list(vizType = "groupedBarPlot"),
    transform = list(
...
slide <- newSlide(
    ...
    title = glue("Pets by location using another palette"),
...
dimension is made from subvariables (as in a MultipleResponse variable or a
...
- `dotplot` (availble for displays of means)
...
editor to perfect the look for one slide and then use it for a whole set of slides.
