...
```{r, eval=FALSE}
class(2015 - ds$birthyr) # it persists as an expression linking the two
```
```{r, echo=FALSE}
class(cr.expr)
```
...
The work gets done without having to pull data off of the server (and so is faster), and the expressions persists and so you do not have to always remember to do re-do it (and so is safer).
