...
For the most part, the **crunch** package will bring the mininum amount of data down from the server. Most statistics and methods work by calculating the values on the server, and then tramitting the results to R rather than sending all the data. In this case the server calculates and transmits the variable summary instead of sending the data. If you ever want to actually access the data in the variable you can call `as.vector(var)`.

Crunch also allows you to store additional metadata in the variable metadata. For instance we can add the survey questions from the scheme to the variable description like this: 
...
Crunch is desgigned for survey data and includes two unique data structures to help analyse survey data: Categorical Arrays and Multiple Response variables. 

#### Categorical Arrays

Surveys commonly asks sets of related questions. For instnace the stack overflow survey includes several questions about the importance of various factors for hiring. Conceptually these questions go together, but because of the limitations of storing data in a table they are presented as separate questions. 
```{r categorical arrays}
ds$ImportantHiringCompanies
ds$ImportantHiringAlgorithms
```

Crunch allows you to package these related questions together into a _Categorical Array_. This is immensely useful when you want to analyse or communicate information about the array of questions. For instance you can use the whole array in a cross tab without having to process dosens of individual variables. 

To create a Categorical Array variable from R we can use the `makeArray` function. 

```{r makeArray}
ds$ImportantHiring <- makeArray(ds[, grep("^ImportantHiring", names(ds))], name = "ImportantHiring")
```
...
We can now work with this array variable just like any other variable. For instance we can generate a cross-tab of all the subvariables and another categorical variable. 

```{r generate cross tab}
crtabs(~ ImportantHiring + TabsSpaces, ds)
```

#### Multiple Response

Crunch also has a special variable type for Multiple Response questions. These are questions that give users a set of options and allow them to select more than one of them. An example of this from the stack overflow survey is asking which language a respodant has worked with. the responses are stored in a single variable with each language delimited by semi colons: 

```{r}
knitr::kable(SO_survey[1:5, "HaveWorkedLanguage", drop = FALSE], row.names = FALSE)
```

To analyse this data without Crunch you would usually go through some data cleaning process to split it up into its constituent parts, and then reshape it into a format which is more amenable to analysis. However, since this is a common way of storing survey answers, Crunch allows you to create a variable directly from the 

```{r makeMRFromText}

ds$HaveWorkedLanguageMR <- 
    
    der <- makeMRFromText(ds$WantWorkLanguage, 
    delim = "; ", 
    name = "WantWorkedMR")
