...
This variable hierarchy can be thought of like a file system on your computer, with files (variables) organized into directories (folders). As such, the main functions you use to manage it are reminiscent of a file system, though they have more verbose aliases in case you're not familiar with the terse names.

* `cd()`, changes directories, i.e. selects a folder (aka `selectFolder()`)
* `mkdir()` makes a directory, i.e. creates a folder (aka `makeFolder()`)
* `mv()` moves variables and folders to a different folder (aka `moveToFolder()`)
* `rmdir()` removes a directory, i.e. deletes a folder (aka `deleteFolder()`)

Like a file system, you can express the "path" to a folder as a string separated by a "/" delimiter, like this:

```r
cd(ds, "Brands/Cars and Trucks/Domestic")
```

If your folder names should legitimately have a "/" in them, you can set a different character to be the path separator. See `?cd` for details. You can also specify paths as a vector of path segments, like

```r
cd(ds, c("Brands", "Cars and Trucks", "Domestic"))
```
...
This uses the `"."` folder "path" to indicate that you're "moving" the object to be in the current folder.
