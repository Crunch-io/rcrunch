...
# Only run when we have access to data (first time building during R CMD BUILD) so
# that we don't have to include data in package because CRAN limits install size
knitr::opts_chunk$set(eval = dir.exists("../vignette-data"))
```
...
library(crunch)
library(httptest)
start_vignette("../vignette-data/fork-and-merge")
```
