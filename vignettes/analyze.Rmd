...
With the `crunch` library, you can harness the power of R to do computations with your datasets in Crunch that would be difficult or impossible to accomplish in a graphical user interface. 

## Crosstabbing
While the web application certainly supports crosstabbing, crosstabbing in R with `crunch` allows you to easily do additional computations on the result. 

`crunch` contains the `crtabs` (Crunch-tabs) function, which largely emulates the design of the `xtabs` function in base R. In essence, you define a formula and provide data in which to evaluate it. In this case, we'll be providing a formula with some variables and a `CrunchDataset` as the data. Dimensions of your crosstab go on the right side of the `~`. For a univariate table of frequencies by education, we can do
...
```{r, eval=FALSE}
weight(ds) # Note that weight() is a function; not the variable called "weight"
```
```{r, echo=FALSE}
print(NULL)
```
...
```{r, eval=FALSE}
crtabs(~ educ + gender, data=ds) # table with weight on
```
```{r, echo=FALSE}
tab2weighted
```
...
```{r, eval=FALSE}
crtabs(~ educ + gender, data=ds, weight=NULL) # now weight off
```
```{r, echo=FALSE}
tab2
```
...
For column proportions, specify "margin=2"" (for by rows use "margin=1"):
...
```{r}
round(100*prop.table(tab2, 2)) # Note "margin" is implicit here
```
...
Not surprisingly, Democrats were less pessimistic about the direction of the country than the general population (given this was during President Obama's tenure).
