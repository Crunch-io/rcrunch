...
All work with data in Crunch requires users to be authenticated. Thus, the first step after loading the *crunch* library in R is to log in:
```{r, eval=FALSE}
...
This will log you in to the Crunch API. All of these parameters can be also be set as R `options` in your .Rprofile so that you can simply `login()`. See `?login` and then google "how to edit .RProfile for dummies" for details.
