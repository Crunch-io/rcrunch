...
The main feature of representing these counts as high dimensional arrays is that it makes a number of computational tasks a lot easier, but the problem is that you can quickly end up with cubes which are difficult for humans to conceptualize. For instance if you created a cube with one MR variable which had 5 categories, one MR variable with 6 categories, and a categorical variable with 10 categories it would have 5 dimensions with 2700 `(5*3 * 6*3 * 10)` entries. This is hard to understand and to print, and as a result we try to figure out the parts of the cube which the user is interested in and print that sub-cube. For the purposes of this document we need three terms for cubes:
...
We resolve this problem by allowing the user to select whether they want the cube to represent the real cube or the printed cube, and make all of the behaviour of those cubes consistent. We can make use of the `@useNA slot` to the cube to control whether the user wanted to interact with the printed cube or the user cube and provide the `showMissing()` and `hideMissing()` functions to control that behaviour. 
