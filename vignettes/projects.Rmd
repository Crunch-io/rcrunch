...
## TODO: print method, and print output
```
...
Let's make some groups and move some datasets into them. To start, ... . We could call `mkdir()` and then `mv()`, but for convenience, `mv()` will create the group specified by your path if it doesn't already exist, so we can create the group and put

<!-- Stuff below here is copypasta from variable-order.Rmd for reference. -->
