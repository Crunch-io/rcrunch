...
with(temp.option(crunch.check.updates=NULL), {
    with_mock_crunch({
        test_that("checkForNewVersion parses github json", {
            expect_identical(checkForNewVersion("foo", "1.5.3"),
                NULL)
            expect_identical(checkForNewVersion("foo", "1.5.1"),
                "1.5.3")
            expect_identical(checkForNewVersion("foo", "1.6.3"),
                NULL)
        })
        ...
        test_that("notifyIfNewVersion messages correctly", {
            expect_message(notifyIfNewVersion("crunch", "foo", "1.5.1"),
    ...
    without_internet({
        test_that("notifyIfNewVersion doesn't hang if GitHub doesn't respond", {
            expect_message(uncached(notifyIfNewVersion("crunch", "foo", "1.5.1")),
                NA)
        })
    })
})
