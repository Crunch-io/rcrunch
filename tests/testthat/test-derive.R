...
with_mock_crunch({
    ...
    birthyrPlus <- CrunchVariable(
        VariableTuple(index_url = "https://app.crunch.io/api/datasets/1/variables/?relative=on",
    ...
    test_that("derivation()<- input validation", {
        expect_error(derivation(ds$birthyr) <- ds$birthyr + 1,
                     paste0("The variable ", dQuote("Birth Year"),
                            " must already be a derived variable."))
        expect_error(derivation(birthyrPlus) <- "not an expression",
                     paste0("The value ", dQuote("not an expression"),
                            " must be a CrunchExpr, got a character instead."))
    })

    test_that("derivation()<- issues a PATCH",{
        expect_PATCH(derivation(birthyrPlus) <- ds$birthyr + 1,
    ...
    test_that("derivation()<-NULL and is.derived()<-FALSE issues a PATCH",{
        expect_PATCH(derivation(birthyrPlus) <- NULL,
                     'https://app.crunch.io/api/datasets/1/variables/birthyrPlus/',
                     '{"derived":false}')
