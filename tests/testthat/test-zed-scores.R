...
test_that("z-scores can calculate to row margins - crunch-cube fixture (axis 1)", {
    ...
    out <-  array(c(-0.6950547, 0.7375224,
                    -0.524633, 0.556688,
                    -1.454419, 1.5432837,
                    1.0896893, -1.1562691,
                    3.2737502, -3.4737757,
                 ...
                 dimnames = list(Gender = c("Male", "Female"),
                                 RespondentIdeology =
                                     c("Very liberal", "Liberal", "Moderate",
                                       "Conservative", "Very Conservative",
                                       "Not sure")))
    expect_equal(z.table(gender_x_ideology, 2), out)
})


test_that("z-scores can calculate to row margins - crunch cube fixture (axis 1)", {
    out <- matrix(
        c(c(-1.35700098973668, 3.39819222765456, 3.47632774910236,
            1.39986424142017, 2.33910237706402, -6.92590429515317,
            -0.237453687452224, 0.736452470486666),
...
test_that("p-values can calculate to columns margins - crunch cube fixture (axis 0)", {
    out <- matrix(c(c(-1.33325107235154, 3.4170985193131, 3.56231261682056,
...
test_that("p-values can calculate to row margins - crunch-cube fixture (axis 1)", {
    out <- matrix(c(c(-1.74780896e-01, 6.78327392e-04, 5.08330650e-04,
...
test_that("z-scores can calculate to column margins - crunch-cube fixture (axis 0)", {
    out <- matrix(c(c(-1.82449424e-01, 6.32923709e-04, 3.67602277e-04,
...
test_that("z-scores can calculate to row margins - whaam fixture", {
    out <- array(c(17.3006739150679, -17.2790610621901,
                   12.1555052876046, -12.1403200324679,
                   -2.61883165552036, 2.6155600821955,
                   -3.12585957287982,  3.12195459533981,
                   -7.73178794867428, 7.72212901884083,
                   -23.9433203846143, 23.9134092110139),
                 dim = c(2, 6),
                 dimnames = list(Admit = c("Admitted", "Rejected"),
                                 Dept = c("A", "B", "C", "D", "E", "F")))
    expect_equal(z.table(admit_by_dept_unweighted, 1), out)
})

test_that("z-scores can calculate to row margins - whaam fixture", {
    out <- array(c(9.80281743121017, -9.71107624617507,
                   -9.80281743121017, 9.71107624617506),
                 dim = c(2, 2),
                 dimnames = list(Admit = c("Admitted", "Rejected"),
                                 Gender = c("Male", "Female")))
    expect_equal(z.table(admit_by_gender_weighted, 1), out)
})

test_that("z-scores can calculate to column margins - whaam fixture", {
    out <- array(c(18.7216214725448, -18.7216214725448,
                   13.3291986335621, -13.3291986335621,
                   -2.67980030430232, 2.67980030430231,
                   -3.19261047229265, 3.19261047229265,
                   -8.09694682104735, 8.09694682104735,
                   -32.0139892315214, 32.0139892315214),
                 dim = c(2, 6),
                 dimnames = list(Admit = c("Admitted", "Rejected"),
                                 Dept = c("A", "B", "C", "D", "E", "F")))
    expect_equal(z.table(admit_by_dept_unweighted, 2), out)
})

test_that("z-scores can calculate to column margins - whaam fixture", {
    out <- array(c(9.75089877074671, -9.75089877074672,
                   -9.72361434000118, 9.72361434000117),
                 dim = c(2, 2),
                 dimnames = list(Admit = c("Admitted", "Rejected"),
                                 Gender = c("Male", "Female")))
    expect_equal(z.table(admit_by_gender_weighted, 2), out)
})

## p-values unweighted
test_that("z-scores can calculate to row margins (unweighted) - whaam fixture (pvalues calc)", {
    out <- matrix(c(c(-0.013867151154616897, 0.0009563833962253909,
                    ...
                    c(0.013871483015614139, -0.0009568966033475235,
...
test_that("z-scores can calculate to columns margins (unweighted) - whaam fixture (pvalues calc)", {
    out <- matrix(c(
        c(-0.013861902224157063, 0.0009542912810769355,
        ...
        c(0.013861902224157951, -0.0009542912810769355,
...
test_that("z-scores can calculate to row margins - whaam fixture (pvalues calc)", {
    out <- matrix(c(c(-0.17478090412248748, 0.0006783274445936183,
                    ...
                    c(0.17610956603574945, -0.0007046838930806487,
...
test_that("z-scores can calculate to columns margins - whaam fixture (pvalues calc)", {
    out <- matrix(c(c(-0.1824494335182465, 0.0006329237585269709,
                    ...
                    c(0.18244943351825338, -0.0006329237585269709,
