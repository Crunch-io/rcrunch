...
with_mock_auth_settings <- function(
  ...
  with(
      ...
      withr::with_envvar(
          env_vars,
          {
              mockery::stub(login, "keyring::key_list", function(service = NULL, keyring = NULL) {
                  if (is.null(keyring_users) & is.null(keyring_pws)) {
                      data.frame(
                          service = character(0), username = character(0), stringsAsFactors = FALSE
                      )
                  } else if(is.null(keyring_users)) {
                      data.frame(service = service, username = "", stringsAsFactors = FALSE)
                  } else {
                      data.frame(
                          service = service, username = keyring_users, stringsAsFactors = FALSE
                      )
                  }
              })
              mockery::stub(login, "keyring::key_get", function(service, username = NULL, keyring = NULL) {
                  if (is.null(username)) {
