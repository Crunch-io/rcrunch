...
with_mock_HTTP({
    ...
    test_that("Box too big message", {
        with_mock(`crunch:::.boxlimit`=function () -1, {
            expect_error(crunchBox(ds[2:5]),
                "4 variables and 0 filters results in too many cubes")
            expect_error(crunchBox(ds[2]),
                "1 variable and 0 filters results in too many cubes")
        })
    })
})
