...
test_that("Basic join validatation (and warning)", {
    expect_warning(
        expect_error(
            joinDatasets(join1, join2, copy = FALSE),
...
with_mock_crunch({
    ...
    test_that("Correct payload without filtering", {
        expect_PATCH(
            join(ds1, ds2, by.x = ds1$birthyr, ds2$birthyr),
    ...
    test_that("Can reference variables by alias", {
        expect_PATCH(
            join(ds1, ds2, by.x = "birthyr", by.y = "birthyr"),
            "https://app.crunch.io/api/datasets/1/joins/",
            testPayload
        )
        expect_PATCH(
            join(ds1, ds2, by = "birthyr"),
            "https://app.crunch.io/api/datasets/1/joins/",
            testPayload
        )
    })
    ...
    test_that("Input validation for join", {
        ...
        expect_error(
            join(ds1, ds2, by.x = ds2[[1]]),
            "by.x must be a variable in x"
        )
        expect_error(
            join(ds1, ds2, by.x = ds1[[1]], by.y = 1),
            "by.y must be a Crunch Variable"
        )
        expect_error(
            join(ds1, ds2, by.x = ds1[[1]], by.y = ds1[[1]]),
            "by.y must be a variable in y"
        )
        expect_error(
            join(ds1, ds2, by.x = ds1$birthyr, by.y = ds2$birthyr, all = TRUE),
            'Option "all" not supported.'
        )
        expect_error(
            join(ds1, ds2, by.x = ds1$birthyr, by.y = ds2$birthyr, all.x = FALSE),
            'Option "all.x=FALSE" not supported.'
        )
        expect_error(
            join(ds1, ds2, by.x = ds1$birthyr, by.y = ds2$birthyr, all.y = TRUE),
    ...
    test_that("Categorical and array variables can't be used as keys", {
        expect_error(
            join(ds1, ds2, by.x = ds1$gender, by.y = ds2$birthyr),
            "by.x must be type numeric or text"
        )
        expect_error(
            join(ds1, ds2, by.x = ds1$birthyr, by.y = ds2$gender),
    ...
    test_that("An invalid alias gives a useful error message", {
        expect_error(
            join(ds1, ds2, by.x = "NOTAVARIABLE"),
            "NOTAVARIABLE does not reference a variable in x"
        )
        expect_error(
            join(ds1, ds2, by.x = ds1$birthyr, by.y = "NOTAVARIABLE"),
