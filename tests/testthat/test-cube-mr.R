...
# c(9928.20954289002, 11524.821237084192, 0.0, 20942.20463208863, 22037.232566079987, 0.0, 9588.843313998908, 9801.254016136965, 0.0, 21051.95287450818, 22193.92389616964, 0.0, 11697.435357575358, 13095.670425525452, 0.0, 21093.329724263043, 22238.63512193253, 0.0, 9782.8995547749, 10531.918128023966, 0.0, 24022.804995331622, 25731.944913511692, 0.0, 4417.596222134318, 3448.380316269752, 0.0, 23843.766861843127, 25534.337452716918, 0.0, 6179.175512581436, 6490.427474934746, 0.0, 23961.052646443855, 25682.95821700546, 0.0)

test_that("as.array on cube with as_selected x cat", {
    expect_equal(as.array(mr_x_cat_wt),
        arrayify(matrix(c(
            ## These numbers are pruned from the JSON, which is row-major, so
            ## use matrix(byrow) to fix
            ...
            ), byrow=TRUE, nrow=6), c("attitudes_recoded_klima_2", "pdl_gender")))
})

test_that("margin.table for as_selected x cat", {
    expect_equal(margin.table(mr_x_cat_wt, 1),
        arrayify(c(21453.03077997421,
                19390.097330135875,
                24793.105783100807,
                20314.817682798865,
                7865.976538404069,
                12669.602987516182), "attitudes_recoded_klima_2"))
    ...
    expect_equal(margin.table(mr_x_cat_wt),
        arrayify(c(30173.5628018302,
                   29907.1232293211,
                   29821.0351538032,
                   23398.2500911551,
                   23774.8956854385,
                   23508.9891365492), "attitudes_recoded_klima_2"))
})
...
test_that("prop.table(cell) for as_selected x cat", {
    ...
    expect_equal(prop.table(mr_x_cat_wt),
        arrayify(matrix(c(
                0.329036700375595, 0.320620717695708,
                0.392254504152701,  0.418103897371069,
                0.185809278853744, 0.262843097025161,
                0.381950958618156,  0.327723062528721,
                0.439142047148397, 0.450115632023491,
                0.14504292098248,  0.27608279697761
            ), byrow=FALSE, nrow=6), c("attitudes_recoded_klima_2", "pdl_gender")))
})
