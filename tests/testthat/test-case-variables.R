...
with_mock_crunch({
    ...
    test_that("Case list validator", {
        ...
        expect_error(ensureValidCase(list(not_right="not")),
                     paste0("each case must have at most an id, name, expression, ",
    ...
    test_that("makeCaseVariable errors gracefully", {
        ...
        expect_error(makeCaseVariable(`Old women`=ds$birthyr < 1950,
            ...
            paste0("can't have case conditions both in ", dQuote("..."),
        ...
        expect_error(makeCaseVariable(cases=list(list(
            ...
            paste0("could not find names for a case; this might be because the cases",
        ...
        expect_error(makeCaseVariable(cases=list(
            ...
            name=""),
            paste0("you can only provide a single else case; you have more than one ",
            "in either"))
        expect_error(makeCaseVariable(cases=list(
            list(expression=ds$gender == "Male", name="Dudes"),
            list(expression="else", name="else1"),
            list(expression=ds$gender == "Female", name="Not Dudes")), name=""),
            paste0("The else case must be the last element of ", dQuote("cases"),
            " or ", dQuote("...")))
        expect_error(makeCaseVariable(cases=list(
            list(expression=ds$gender == "Male", name="Dudes"),
            list(expression="else", name="name", id=99999999)), name=""),
            paste0("id must be less than 32,768, this might be a result of too many ",
            "cases being used."))
        expect_error(makeCaseVariable(cases=list(
            list(expression=ds$gender == "Male", name="Dudes"),
        ...
        expect_error(
            makeCaseVariable(
                ...
                name="Super clever segmentation"),
            paste0('there are duplicate condition expressions provided: ',
