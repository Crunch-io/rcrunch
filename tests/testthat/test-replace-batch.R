    ...
    with(test.authentication, {
        ...
        with(test.dataset(df, "d1"), {
            with(test.dataset(df2, "d2"), {
                test_that("Show that the two datasets are different", {
                ...
                test_that("A very similar batch can be replaced", {
                    ...
                    expect_identical(ncol(d1), ncol(d2))
                    expect_identical(as.vector(d1$v3), as.vector(d2$v3))
                    expect_identical(as.vector(d1$v4), as.vector(d2$v4))
                    expect_true(is.null(d1$v6))
