...
test:
	...
	R --slave -e 'library(testthat); setwd(file.path(.libPaths()[1], "rcrunch", "tests")); system.time(test_check("crunch", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
test-ci:
	R --slave -e '.libPaths(Sys.getenv("R_LIB")); options(test.user=Sys.getenv("R_TEST_USER"), test.pw=Sys.getenv("R_TEST_PW"), test.api=Sys.getenv("R_TEST_API")); library(testthat); sink(file="rcrunch.tap"); setwd(file.path(.libPaths()[1], "rcrunch", "tests")); test_check("crunch", reporter="tap"); sink()'
