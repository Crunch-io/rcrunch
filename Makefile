...
test-ci:
	...
	R --slave -e '.libPaths(Sys.getenv("R_LIB")); Sys.setlocale(locale="en_US.UTF-8"); options(test.user=Sys.getenv("R_TEST_USER"), test.pw=Sys.getenv("R_TEST_PW"), test.api=Sys.getenv("R_TEST_API")); library(testthat); sink(file="rcrunch.tap"); setwd("pkg/tests"); test_check("rcrunch", reporter="tap"); sink()'
