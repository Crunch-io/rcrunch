...
To create new datasets, multiple paths exist. In the web application, you can upload files to create datasets. From rcrunch, there are two methods for creating datasets: one for `data.frames` and one for files of other (non-R) formats. 
...
Datasets can also be deleted permanently. This action cannot be undone, so it should not be done lightly. rcrunch provides two ways to delete a dataset: a `delete` method on a dataset object, like
