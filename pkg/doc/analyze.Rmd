...
Additional dimensions are added with `+`. For a two-way table of gender and education, 

```{r, eval=FALSE}
tab2 <- crtabs(~ gender + educ, data=ds)
tab2
```
```{r, echo=FALSE}
tab2
...
```{r, eval=FALSE}
crtabs(~ gender + educ, data=ds, weight=NULL)
```
```{r, echo=FALSE}
tab2
```
...
Looks like most people surveyed thought that the U.S. is on the wrong track.
...
```
```{r, echo=FALSE}
print(summary.snowdenleakapp.var)
```
